...
def verify(payload):
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(payload)) as z:
            ...
            if isinstance(manifest,dict) and manifest.get('schema')=='divinity2.model-source-package/4':
                from .physics_carrier import verify_members
                verify_members(z,manifest)
                return manifest
            if isinstance(manifest,dict) and manifest.get('schema')=='divinity2.model-source-package/3':
                from .physics_carrier import verify_members
                verify_members(z,manifest)
                return manifest
...
def open_preview(payload):
    ...
    if manifest.get('schema') in ('divinity2.model-source-package/3','divinity2.model-source-package/4'):
        # Builder preview is visual-only; native physics has no interpretation.
        with zipfile.ZipFile(io.BytesIO(payload)) as z:
            return open_preview(z.read('model.d2model'))
