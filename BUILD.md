# Building 0.2.8 from source

This checkout contains version 0.2.8. The existing source-identity,
...
.\.venv\Scripts\python.exe -m dks_patch_builder check
...
The resulting `DKSPatchBuilder` folder contains the EXE, `_internal` runtime,
...
```powershell
.\.build\candidate-output\DKSPatchBuilder\DKSPatchBuilder.exe check --report .build\check-result.json
```
