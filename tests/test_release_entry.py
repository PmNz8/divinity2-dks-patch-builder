...
from dks_patch_builder.desktop import ABOUT_TEXT, FOOTER_TEXT, WINDOW_TITLE, BuilderTkApp
from dks_patch_builder.version import APP_NAME, COPYRIGHT, LICENSE_NAME, PROFILE_URL, VERSION
...
class ReleaseEntryTests(unittest.TestCase):
    ...
    def test_version_literals_and_frontend_branding(self) -> None:
        ...
        self.assertIn("filevers=(0, 2, 8, 0)", version_resource)
        self.assertIn("prodvers=(0, 2, 8, 0)", version_resource)
        self.assertEqual(APP_NAME, "DKS Patch Builder")
        ...
        self.assertIn(VERSION, WINDOW_TITLE)
    ...
    def test_check_report_is_new_file_only_and_refuses_overwrite(self) -> None:
        payload = {
            ...
            "python": "3.12.6",
            "frozen": False,
            "modules": ["dks_patch_builder"],
        }
        with tempfile.TemporaryDirectory(prefix="dks-patch-builder-release-") as raw:
    ...
    def test_build_validates_platform_and_new_absolute_directories(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="dks-patch-builder-build-") as raw:
            parent = Path(raw)
    ...
    def test_pyinstaller_command_has_required_flags_and_exclusions(self) -> None:
        ...
        self.assertEqual(command[:9], [
            sys.executable, "-m", "PyInstaller", "--noconfirm", "--onedir", "--windowed", "--noupx", "--name", "DKSPatchBuilder"
        ])
    ...
    def test_build_metadata_uses_relative_hash_manifest_without_private_paths(self) -> None:
        with tempfile.TemporaryDirectory(prefix="dks-patch-builder-build-") as raw:
            bundle = Path(raw) / "DKSPatchBuilder"
            bundle.mkdir()
            (bundle / "DKSPatchBuilder.exe").write_bytes(b"synthetic")
            context = build_script.BuildContext(
            ...
            self.assertTrue(any(item["path"] == "DKSPatchBuilder.exe" for item in build_info["manifest"]))
